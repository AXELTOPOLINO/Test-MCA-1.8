...
public class BakedTestTileModel implements IBakedModel, ISmartBlockModel, ISmartItemModel
{
	...
	private List<BakedQuad> getQuads(float offsetX, float offsetY, float offsetZ, MCAModelBox box, EnumFacing face)
	{
		...
		int ord = face.getOpposite().ordinal();
		int i = -1;
		switch (ord)
		{
			case 1:
		...
		for (i = 0; i < 4; i++)
		{
			float x = (float) ((vs[3 - i].vector3D.xCoord + offsetX) / 16.0F) + 0.5F;
			float y = (float) ((vs[3 - i].vector3D.yCoord + offsetY) / 16.0F) + 1.0F;
			float z = (float) ((vs[3 - i].vector3D.zCoord + offsetZ) / 16.0F) + 0.5F;
			float u = vs[3 - i].texturePositionX * 16;
			float v = vs[3 - i].texturePositionY * 16;
